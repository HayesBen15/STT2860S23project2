...
author: "Ben Hayes and David Carr"
...
```{r pivotandtidy}
TidyStreamingData <- StreamingData %>%
  pivot_longer(
  ...
  filter(YesNo > 0) %>%
  separate(col = IMDb, into = c("Actual_IMDb_Rating", "IMDb_Possible_Rating"), sep = "/") %>%
  separate(col = Rotten.Tomatoes, into = c("Actual_Rotten_Tomatoes_Rating", "Rotten_Tomatoes_Possible_Rating"), sep = "/") %>%
  mutate(
    Actual_IMDb_Rating = as.numeric(Actual_IMDb_Rating),
    IMDb_Possible_Rating = NULL,
    Actual_Rotten_Tomatoes_Rating = as.numeric(Actual_Rotten_Tomatoes_Rating),
    Rotten_Tomatoes_Possible_Rating = NULL
  )
...
```{r savedatafiles}
save(StreamingData, disneydata2, file = "../data_tidy/TIDYData.RData")
```
...
####Unfortunately, this section is not filled out due to the passing of our groupmate Jacob Reber
...
```{r ratingsbarplot}
disneydata3 <- filter(disneydata2, !is.na(rating))
ggplot(data = disneydata2, aes(y = rating, fill = type)) +
  geom_bar(position = "dodge", width = 0.8) +
  labs(
    title = "Frequencies of Rating Categories for Programs on Disney+",
    y = "Rating",
    x = "Count",
    fill = "Program Type"
  ) +
...
```{r servicequantity}
ggplot(TidyStreamingData, aes(x = Service)) +
geom_bar(fill = "steelblue", width = .7) +
theme_classic() +
labs(title = "Streaming Service Content Comparison", x = "Streaming Service", y = "Amount of Content by Count")
```
...
```{r serviceratings}
ggplot(TidyStreamingData, aes(x = Actual_IMDb_Rating)) +
  geom_histogram(fill = "red", binwidth = .5) +
  facet_wrap(~Service, scales = "free") +
  theme_classic() +
  labs(title = "IMDb Ratings Across Streaming Platforms", x = "IMDb Rating", y = "Amount of Shows Per Score")
...
1) ####Unfortunately, this section is not filled out due to the passing of our groupmate Jacob Reber


2) ####Unfortunately, this section is not filled out due to the passing of our groupmate Jacob Reber


3) ####Unfortunately, this section is not filled out due to the passing of our groupmate Jacob Reber


4) ####Unfortunately, this section is not filled out due to the passing of our groupmate Jacob Reber


5) ####Unfortunately, this section is not filled out due to the passing of our groupmate Jacob Reber
